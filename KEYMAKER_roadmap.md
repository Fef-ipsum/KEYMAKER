...
> Pour lancer l'app : double-clic sur `keymaker-app/start.bat`. Tu arrives sur ton **dernier flash** ; **Run** (ou `Ctrl+Enter`) joue le son, **Précédent / Suivant** parcourent les flashs du module en traversant les chapitres, le bouton **Parcours** ouvre la carte — désormais avec un **sélecteur Module 1 / 2 / 3** — et le bouton **⚙ Réglages** ouvre la configuration (taille de texte, thème éditeur, numéros de ligne, URL Pi, reprendre à zéro). **Trois modules** maintenant : Module 1 (live coding, 26 flashs), Module 2 (solfège, 25 flashs) et Module 3 (Connexion Guitare, 25 flashs) — **76 flashs** au total.
...
- **Réconcilié le 5 juin** (cette session) : app **rebuildée** sur la machine de Felix (3 modules + thème clair, bundle `index-NqFEHDkR.js`, 34 modules), README + roadmap mis à jour, brief `KEYMAKER_chantier9.md` créé. ⚠️ **Reste** : valider les 25 codes Strudel à l'oreille + étoffer le brief avec les décisions de contenu.

### ⬜ Chantiers contenu — Modules 4 à 6
À définir au fur et à mesure, comme les Modules 1-3. **Pas un prérequis pour coder.**
Pistes (cf. `KEYMAKER_architecture.md`) : 4 Théorie Audio & Effets · 5 Informatique Musicale · 6 Composition & Projets.
...
- Thème par défaut au 1ᵉʳ lancement : **Void** (sombre). **Light (clair) livré** (Chantier 8) — bascule dans ⚙ Réglages → Apparence, persistée. Matrix : plus tard.
