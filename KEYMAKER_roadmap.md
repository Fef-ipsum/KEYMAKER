...
- ✅ **Chantier Electro — Électro en fil rouge + Module 7 FAIT (6 juin 2026, en autonomie)** : (1) **M1–M6 recolorés par genre** — chaque module ancré dans un genre électro (M1 **techno** · M2 **trance** · M3 **industrial/EBM** · M4 **acid/psy** · M5 **ambient & D&B** · M6 **house**) : intro, bilan-clé et exercice libre final retouchés (pas de réécriture, recoloriage ciblé). (2) **Module 7 « Genres & Styles »** — 5 chapitres, **25 flashs (7.1→7.25)** : house · techno · drum & bass · trance/psy · ambient — un VRAI morceau complet par genre (anatomie, groove/dynamique, son signature, `arrange`, projet final). Défini dans **`src/module7.js`** (≠ tout-dans-lessons.js : le brief disait `module7.js`/`modules.js`, réconcilié en fichier dédié importé dans `lessons.js`), `modules = [module1..module7]`, **aucune modif `App.jsx`** (tout est dynamique). Code vérifié contre `KEYMAKER_strudel_reference.md`. **Total : 176 flashs.** Le dernier flash de Keymaker = **7.25** (pièce ambient générative). Build propre (bundle `index-pfm8e5dE.js`, 392 kB) + **intégrité 1190/0** + **navigation 9/9** + sentinelles (10/10) dans le bundle. → `KEYMAKER_chantier_electro.md` (réalisé) · `KEYMAKER_chantier_electro_brief.md` (brief de design)
...
- ✅ **Chantier 31 — Intégration PO-33 KO — FAIT (6 juin 2026, en autonomie)** : (1) **Tranche 1** — dossier `public/sounds/po33/` + **Module 8 « Hardware & PO-33 »** (2 ch / **5 flashs** : pipeline de sampling, kit `samples()`, chop de break · sync 2 PPQN · duo live), `modules=[module1..module8]` → **181 flashs**. (2) **Tranche 2** — générateur **◧ Sync PO-33** : clics Web Audio **2 PPQN** sur le **canal GAUCHE**, calés sur `scheduler.cps`/`now()` (cœur pur **`syncCore.js`** — **37/37** + simulation **6/6**), panneau thème-aware sous l'éditeur, réglage persisté `keymaker:settings.posync`. Protocole **corrigé** vs roadmap (2 PPQN, canal GAUCHE, mode **SY2** côté PO). Build **45 modules** + sentinelles + dist byte-vérifié. **Reste : test audio de bout en bout avec Felix (câble Y séparateur L/R).** → `KEYMAKER_chantier31.md`
...
- 📋 **Audit complet de l'app (10 juin 2026)** → `KEYMAKER_analyse_2026-06-10.md` : forces, bugs (B1–B8), 14 chantiers proposés (34–47) avec priorités. Point d'entrée des prochains chantiers.
- ✅ **Chantier 34 — L'app servie par le Pi — FAIT (10 juin 2026, en autonomie)** : l'app est accessible sur **`https://personal-os.tailac998e.ts.net/keymaker/app/`** depuis tout appareil Tailscale (tablette, téléphone) — servie par le module keymaker (`@fastify/static`, volume `webapp/`, repli SPA), sous la route Caddy `/keymaker*` existante → **zéro changement d'infra partagée**. `DEFAULT_PI_URL` auto (même origine) sur un nouvel appareil. Deux builds : `dist/` (local, inchangé) + `dist-pi/` (base `/keymaker/app/`). Vérifié via Caddy/HTTPS (index, bundle, vendor, manifest, SPA, health, SSE). ⚠️ Responsive < 880 px à faire (C40). → `KEYMAKER_chantier34.md`
- ✅ **Chantier 35 — Mémoire de Sati assainie + Opus — FAIT (10 juin 2026, en autonomie)** : **bug B1 corrigé** — le bloc `<contexte_app>` partait dans le journal/embeddings (21 entrées polluées, rappel sémantique dilué). Désormais `{message, context}` séparés (le Pi n'embedde QUE la question) + nettoyage & ré-embedding des entrées existantes. **B2/B3** : `max_tokens` par mode (600/1500/4000) + 3ᵉ mode **« Profond » (Opus)** dans le composeur ; « ✏️ Crée une leçon » passe en deep (plus de troncature). **B8** : retry léger du fetch initial. ⚠️ Piège noté : Voyage ≈ 3 req/min (ré-embedding en rafale = échec silencieux). → `KEYMAKER_chantier35.md`
- ⏸️ **Chantier 28 — Notifications PWA de rappel** : **EN PAUSE (décision Felix, 10 juin 2026 : « pas de notifications pour l'instant, j'en ai déjà beaucoup »)**. Idem pour l'idée « rappels ntfy » de l'audit. À ne proposer à nouveau que si Felix le demande.
...
> ✅ **Réalisé.** Tranche 1 (sampling → `samples()`, **Module 8**, 5 flashs) + Tranche 2 (générateur **◧ Sync PO-33** : clics **2 PPQN** sur le **canal gauche**, `syncCore.js` **37/37** + sim **6/6**). Protocole corrigé : 2 PPQN, canal GAUCHE, PO en **SY2**. Compromis stéréo (mono pendant la sync) documenté. **181 flashs.** Détails : `KEYMAKER_chantier31.md`. La description ci-dessous = l'intention initiale.
...
*Dernière mise à jour : 6 juin 2026 — **Chantier 31 « Intégration PO-33 KO » FAIT (en autonomie)** : Module 8 « Hardware & PO-33 » (sampling + sync live → **181 flashs / 8 modules**) + générateur **◧ Sync PO-33** (clics **2 PPQN** canal gauche, cœur pur `syncCore.js` **37/37**, build **45 modules**, dist byte-vérifié). Protocole corrigé (2 PPQN, canal GAUCHE, SY2) ; **reste le test audio avec Felix (câble Y)**. → `KEYMAKER_chantier31.md`. — Précédemment, **Chantier 26 « Visualiseur de pattern » FAIT (en autonomie)** : panneau `◫ Visualiseur` sous l'éditeur — grille rythmique animée (voies × 16 pas + **curseur calé sur `scheduler.now()`**) qui LIT le REPL (`editor.pattern.queryArc`), **éditeur jamais recréé**, thème-aware, réglage persisté `keymaker:settings.viz` (OFF par défaut), visible en Mode Focus. Nouveaux fichiers : **`vizCore.js`** (cœur pur, **32/32** node) + **`PatternViz.jsx`** ; `App.jsx`/`styles.css` étendus. Build **42 modules**, bundle `index-BdUZ88b0.js` (398,3 kB) + CSS `index-CcUo1K63.css` — **SSR 8/8** + intégrité **176 flashs** + sentinelles, livré **byte-identique** sur le mont. **Au passage, repo réparé** (corruption du pont) : `lessons.js` HEAD tripliqué → dédupliqué, `styles.css`/`roadmap.md` tronqués → restaurés, `close_session.bat` auto-répare un `index.lock` périmé. Détails : `KEYMAKER_chantier26.md`. **Reste : valider le visualiseur à l'œil (Felix).** Avant ça : Chantier Electro (Module 7 « Genres & Styles » → **176 flashs / 7 modules**) + Chantiers 16/20/21/22/24/27 en autonomie ; historique complet dans les sections ci-dessus.*